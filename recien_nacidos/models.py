# hospital/recien_nacidos/models.py
...
class RecienNacido(models.Model):
    """
    Modelo de Recién Nacido (Módulo 3: Neonatología).
    
    Este modelo almacena la ficha clínica inicial del bebé.
    Se vincula a un 'Parto', lo que permite trazar la relación Madre -> Parto -> Hijo.
    
    Características Clave:
    - Identificación única: Genera un código 'RN-XXXX' automático.
    - Test de Apgar: Almacena el desglose (0,1,2) para calcular el total automáticamente.
    - Indicadores de Calidad: Lactancia, Apego, Profilaxis (Requeridos por el MINSAL).
    """
    
    SEXO_CHOICES = [('M', 'Masculino'), ('F', 'Femenino'), ('I', 'Indeterminado')]
    CONDICION_CHOICES = [('vivo', 'Vivo'), ('mortinato', 'Mortinato')]

    # --- TABLAS DE PUNTUACIÓN APGAR ---
    # El test de Apgar evalúa 5 criterios (Frecuencia, Respiración, Tono, Reflejos, Color).
    # Cada criterio tiene un puntaje de 0, 1 o 2.
    ...
    # --- IDENTIFICACIÓN ---
    # Código único para brazaletes (generado en save())
    ...
    # --- APGAR MINUTO 1 (Evaluación inmediata) ---
    ...
    # --- APGAR MINUTO 5 (Evaluación de adaptación) ---
    ...
    # --- TOTALES APGAR ---
    # Campos numéricos calculados automáticamente en el método save().
    # Se usan para estadísticas y alertas de riesgo.
    ...
    # Datos de derivación (si requiere traslado a otro centro)
    ...
    def save(self, *args, **kwargs):
        ...
        1. Genera un ID único (ej: RN-A1B2C3D4) si no existe.
        2. Calcula automáticamente la suma del puntaje Apgar (1' y 5') sumando
           los 5 componentes individuales para evitar errores humanos de cálculo.
        """
        if not self.codigo_unico:
            self.codigo_unico = f"RN-{uuid.uuid4().hex[:8].upper()}"
        
        # Cálculo de seguridad del Apgar
    ...
    def tiene_registros_completos(self):
        """Verifica datos mínimos antropométricos para el alta."""
